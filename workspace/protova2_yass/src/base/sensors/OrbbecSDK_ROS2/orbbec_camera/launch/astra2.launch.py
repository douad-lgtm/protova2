...
def load_parameters(context, args):
    ...
    config_file = os.path.join(package_dir, 'config', 'astra2.yaml')
    
    if os.path.exists(config_file):
        yaml_data = load_yaml(config_file)
        # Extract parameters from nested structure
        if 'orbbec_ros' in yaml_data and 'camera_parameters' in yaml_data['orbbec_ros']:
            params = yaml_data['orbbec_ros']['camera_parameters']
                ...
                if isinstance(values, dict):
                    for key, val in values.items():
                        if isinstance(val, dict):
                            # Handle nested dicts like color_auto_ae
                            for nested_key, nested_val in val.items():
                                yaml_params[nested_key] = nested_val
                        else:
                            yaml_params[key] = val
            
            default_params = merge_params(default_params, yaml_params)
    
    skip_convert = {'usb_port', 'serial_number', 'product_id'}
    result = {}
    for key, value in default_params.items():
        if key in skip_convert:
            result[key] = value
        elif 'enable_pub_plugins' in key:
            # Handle image_transport plugin lists
            if isinstance(value, str):
...
def generate_launch_description():
    ...
    args = [
        ...
        # Image transport plugins for color
        DeclareLaunchArgument('color.image_raw.enable_pub_plugins', default_value='["image_transport/compressed", "image_transport/raw"]'),
        ...
        # Image transport plugins for depth
        ...
        # Image transport plugins for IR
        DeclareLaunchArgument('ir.image_raw.enable_pub_plugins', default_value='["image_transport/compressed", "image_transport/raw"]'),
