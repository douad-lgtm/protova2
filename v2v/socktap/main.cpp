...
int main(int argc, const char** argv)
{
    ...
    options.add_options()
        ...
        ("denm-trigger-port", po::value<unsigned>()->default_value(9002), "UDP port to trigger a DENM (any datagram -> 1 DENM).")
        ("denm-cause", po::value<long>()->default_value(99), "DENM cause code (99=dangerousSituation).")
        ("denm-subcause", po::value<long>()->default_value(2), "DENM sub-cause code (2=emergencyElectronicBrakeEngaged).")
        ("benchmark", "Enable benchmarking")
        ("applications,a", po::value<std::vector<std::string>>()->default_value({"ca"}, "ca")->multitoken(), "Run applications [ca,den,hello,benchmark]")
        ...
        for (const std::string& app_name : vm["applications"].as<std::vector<std::string>>()) {
            ...
            if (app_name == "ca") {
                ...
                std::unique_ptr<DenmApplication> den {
                    new DenmApplication(*positioning, trigger.runtime(), io_context,
                                        vm["denm-trigger-port"].as<unsigned>())
                };
                den->set_station_id(vm["station-id"].as<unsigned>());
                den->set_cause(vm["denm-cause"].as<long>(), vm["denm-subcause"].as<long>());
                den->print_received_message(vm.count("print-rx-denm") > 0);
                den->print_generated_message(vm.count("print-tx-denm") > 0);
