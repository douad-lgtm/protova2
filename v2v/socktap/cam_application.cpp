...
void CamApplication::on_timer(Clock::time_point)
{
    ...
    // Cap : depuis la position (course), en unites 0.1 degre depuis le Nord.
    // Non renseigne (NaN) -> valeur "unavailable".
    const double heading_deg = position.course.value().value();
    if (std::isfinite(heading_deg)) {
        double h = std::fmod(heading_deg, 360.0);
        if (h < 0.0) h += 360.0;
        long hv = std::lround(h * 10.0);
        if (hv < 0) hv = 0;
        if (hv > 3600) hv = 3600;
        bvc.heading.headingValue = hv;
    } else {
        bvc.heading.headingValue = HeadingValue_unavailable;
    }
    bvc.heading.headingConfidence = HeadingConfidence_equalOrWithinOneDegree;

    // Vitesse : depuis la position (speed), en unites 0.01 m/s.
    const double speed_mps = position.speed.value().value();
    if (std::isfinite(speed_mps)) {
        long sv = std::lround(speed_mps * 100.0);
        if (sv < 0) sv = 0;
        if (sv > 16382) sv = 16382;
        bvc.speed.speedValue = sv;
    } else {
        bvc.speed.speedValue = SpeedValue_unavailable;
    }
