...
sudo ip route del default via 172.16.48.5 dev "$IFACE_5G" 2>/dev/null || true
...
# PC workspace lives at ~/protova2/protova2_yass (verified 2026-06-10: the only
# copy with ros_g29_force_feedback built; ~/Desktop copies are not built)
if [ -f "$HOME/protova2/protova2_yass/install/setup.bash" ]; then
    ...
    echo "WARNING: ~/protova2/protova2_yass workspace not found — force feedback will fail (Package not found)"
fi

# Kill stale router AND nodes from previous runs (otherwise duplicates pile
# up: observed 5 joy_node publishers on /joy after repeated relaunches, and a
# 2nd router would race for port 7447)
pkill -x rmw_zenohd 2>/dev/null
pkill -f "ros2 run joy joy_node" 2>/dev/null
pkill -x joy_node 2>/dev/null
pkill -f "ros2 run ros_g29_force_feedback" 2>/dev/null
pkill -x g29_force_feedback 2>/dev/null
sleep 2
...
echo "Keep this terminal open (Ctrl+C stops everything)."

# Keep script attached to its children: without this the backgrounded nodes
# are orphaned on exit and accumulate as stale duplicates across relaunches.
